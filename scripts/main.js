...
("use strict");
{
    window.DOMElementHandler = class DOMElementHandler extends self.DOMHandler {
        ...
        _OnCreate(e) {
            ...
            focusElem.addEventListener("focus", (e) =>
                this._OnFocus(elementId)
            );
        ...
        _OnSetFocus(e) {
            const elem = this._GetFocusElement(
                this._elementMap.get(e["elementId"])
            );
...
("use strict");
{
    ...
    const supportedAudioFormats = {
        ...
        "audio/ogg; codecs=opus": !!tmpAudio.canPlayType(
    ...
    window.RuntimeInterface = class RuntimeInterface {
        constructor(opts) {
            ...
            if (this._exportType === "cordova" && this._useWorker)
                if (isAndroid) {
                    const chromeVer = /Chrome\/(\d+)/i.exec(
                        navigator.userAgent
                    );
        ...
        async _InitWorker(opts, port2) {
            ...
            if (this._exportType === "cordova")
                for (
                    let i = 0, len = opts.projectScripts.length;
                    i < len;
                    ++i
                ) {
                    ...
                    if (
                        ...
                        info[1] = await this._MaybeGetCordovaScriptURL(
                            originalUrl
                        );
                }
        ...
        async _InitDOM(opts, port2) {
            ...
            let engineScripts = opts.engineScripts.map((url) =>
                typeof url === "string"
                    ? new URL(url, this._baseUrl).toString()
                    : url
            );
            ...
            const runtimeOpts = Object.assign(
        ...
        async _OnCordovaFetchLocalFile(e) {
            ...
            switch (e["as"]) {
                ...
                case "buffer":
                    return await this.CordovaFetchLocalFileAsArrayBuffer(
                        filename
                    );
                default:
    ...
    self.JobSchedulerDOM = class JobSchedulerDOM {
        ...
        async Init() {
            ...
            const dispatchWorkerScriptUrl =
                this._runtimeInterface._GetWorkerURL(
        ...
        *audioInstancesByTag(tag) {
        ...
        async _OnLoadState(e) {
            ...
            for (const fxChainData of Object.values(e["effects"]))
                promises.push(
                    Promise.all(fxChainData.map((d) => this._AddEffect(d)))
                );
            await Promise.all(promises);
            await Promise.all(
                e["playing"].map((d) =>
                    this._LoadAudioInstance(d, saveLoadMode)
                )
            );
            this._MaybeStartTicking();
        }
        ...
        static DbToLinear(x) {
            return Math.max(
                Math.min(self.AudioDOMHandler.DbToLinearNoCap(x), 1),
                0
            );
        }
        ...
        static LinearToDb(x) {
            return self.AudioDOMHandler.LinearToDbNoCap(
...
("use strict");
{
    self.C3AudioBuffer = class C3AudioBuffer {
        ...
        CreateInstance(tag) {
            if (this._api === "html5")
                return new self.C3Html5AudioInstance(
                    this._audioDomHandler,
                    this,
                    tag
                );
            else
                return new self.C3WebAudioInstance(
                    this._audioDomHandler,
                    this,
                    tag
                );
    ...
    self.C3WebAudioBuffer = class C3WebAudioBuffer extends self.C3AudioBuffer {
        ...
        async _Fetch() {
            ...
            if (
                ...
                this._audioData =
                    await iRuntime.CordovaFetchLocalFileAsArrayBuffer(
                        this._url
                    );
            else {
    ...
    self.C3Html5AudioInstance = class C3Html5AudioInstance extends (
        ...
        _UpdatePlaybackRate() {
            ...
            try {
                ...
                console.warn(
                    `[Construct 3] Unable to set playback rate '${r}':`,
                    err
                );
            }
        }
        Seek(pos) {
            ...
            try {
                ...
                console.warn(
                    `[Construct 3] Error seeking audio to '${pos}': `,
                    err
                );
            }
        }
        ...
        GetPlaybackTime() {
            ...
            if (this._isPaused) ret = this._resumePosition;
                ...
                ret =
                    ...
                    (this.GetCurrentTime() - this._playStartTime) *
                        this._playbackRate;
            if (!this._isLooping) ret = Math.min(ret, this.GetDuration());
            return ret;
        }
...
("use strict");
{
    ...
    self.C3AudioDistortionFX = class C3AudioDistortionFX extends AudioFXBase {
        ...
        _SetDrive(drive, makeupgain) {
            ...
            this._postGain["gain"]["value"] =
                Math.pow(1 / drive, 0.6) * makeupgain;
        }
    ...
    const HANDLER_CLASS = class TouchDOMHandler extends self.DOMHandler {
        ...
        async _RequestMotionPermission() {
            if (
                ...
                return true;
            try {
                const state = await self["DeviceMotionEvent"][
                    "requestPermission"
                ]();
                return state === "granted";
            } catch (err) {
                console.warn(
                    "[Touch] Failed to request motion permission: ",
                    err
                );
                return false;
            }
...
("use strict");
{
    ...
    class C3BannerAd {
        constructor(id, size, position, resolver) {
            ...
            if (!position)
                return Failure(resolver, "Ad position not specified");
            this._adUnitId = id;
        ...
        async Show(resolver) {
            ...
            if (this._adLoading)
                return Failure(resolver, "banner ad still loading");
            if (this._adShowing)
                return Success(resolver, "banner ad already shown");
            await this._ad["show"]();
...
("use strict");
{
    ...
    class C3InterstitialAd {
        ...
        async Show(resolver) {
            if (!this._ad)
                return Failure(resolver, "interstitial ad not created");
            if (this._adShowing)
    ...
    class C3RewardedInterstitialAd {
        ...
        async Show(resolver) {
            ...
            if (this._adShowing)
                return Success(
                    resolver,
                    "rewarded interstitial ad already shown"
                );
            const show_ad = async () => {
                if (this._loadSuccess) {
                    _AddShowListeners.call(this, resolver);
...
("use strict");
{
    ...
    const Success = C3AdUtils.Success;
    const Failure = C3AdUtils.Failure;
    const IOS = self["cordova"]
        ? self["cordova"]["platformId"] === "ios"
        : false;
                ...
                (error) => {
                    if (error)
                        if (typeof error === "object")
                            Failure(resolver, error.message);
    ...
    async function _RequestInfoUpdateForTesting(debug, debugLocation, onError) {
        ...
        if (!!debug) {
            switch (debugLocation) {
                ...
                case "NOT_EEA": {
                    debugGeography =
                        self["consent"]["DebugGeography"]["NotEEA"];
                    break;
                }
                default: {
                    debugGeography =
                        self["consent"]["DebugGeography"]["Disabled"];
                    break;
                }
            }
...
("use strict");
{
    ...
    addHandler("ShowRewardedAdvert", async (fn) => {
        if (!rewardedAd)
            return C3AdUtils.Failure(fn, "rewarded ad not created");
        rewardedAd.Show(fn);
    });
    addHandler("CreateRewardedInterstitialAdvert", async (id, debug, fn) => {
        if (rewardedInterstitialAd)
            return C3AdUtils.Success(
                fn,
                "rewarded interstitial ad already created"
            );
        if (!(await C3AdUtils.StartAdmobPlus(fn, debug))) return;
        const adPersonalization = await C3Consent.GetAdPersonalization();
...
("use strict");
{
    ...
    const isReady = async (resolver) => {
        ...
        if (!init) {
            ...
            self["adBreak"] = self["adConfig"] = (o) =>
                self["adsbygoogle"].push(o);
            init = true;
        }
    ...
    const suspendRuntime = (s) => {
        const runtimeInterface = api["domSideHandler"].GetRuntimeInterface();
        runtimeInterface.PostToRuntimeComponent("runtime", "visibilitychange", {
            hidden: true,
        });
    };
    const resumeRuntime = (s) => {
        const runtimeInterface = api["domSideHandler"].GetRuntimeInterface();
        runtimeInterface.PostToRuntimeComponent("runtime", "visibilitychange", {
            hidden: false,
        });
            ...
            adBreakDone: (placementInfo) => {
                switch (placementInfo["breakStatus"]) {
                    ...
                    case "ignored": {
                        C3AdUtils.Failure(
                            fn,
                            "rewarded interstitial ad ignored"
                        );
                        break;
                    }
                    default: {
...
("use strict");
{
    ...
    const HANDLER_CLASS = class MobileAdvertHandler extends self.DOMHandler {
        ...
        async _CallMethod(name, data) {
            ...
            if (!api["domSideHandler"]) api["domSideHandler"] = this;
            if (!api) {
                if (!hasShownWarning) {
    ...
    const HANDLER_CLASS = class BrowserDOMHandler extends self.DOMHandler {
        ...
        _OnGetInitialState(e) {
            ...
            return {
                ...
                isConstructArcade:
                    typeof window["is_scirra_arcade"] !== "undefined",
            };
        }
